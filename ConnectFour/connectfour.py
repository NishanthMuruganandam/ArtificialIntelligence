...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def __init__(self, player1_callback, player2_callback, board = ConnectFourBoard(), time_limit = 10,game=False):
    ...
    def run_game(self, verbose=True):
        ...
        while not win_for_player and not self._board.is_tie():            
            for callback, id, symbol in ( player1, player2 ):
                ...
                if self.tokenGame:
                        ...
                        print('Hey token size is ' + str(self.get_board().num_tokens_on_board()))
                        if self.tokenSize == self.get_board().num_tokens_on_board():
                            player1Length = self.get_board().longest_chain(self.get_board().get_current_player_id())
                            player2Length = self.get_board().longest_chain(self.get_board().get_other_player_id())
                            if player1Length == player2Length:
                                print "It's a tie!  No winner is declared."
                                return 0 
                            winner = self.get_board().get_current_player_id() if player1Length > player2Length else self.get_board().get_other_player_id();               
                            self._do_gameend(winner)
                            return
            
                if self._board.is_game_over() and not self.tokenGame:
        ...
        if win_for_player != 0 and self._board.is_tie() and not self.tokenGame:
            print "It's a tie!  No winner is declared."
            return 0
        elif not self.tokenGame:
...
def run_game(player1, player2, k1=4,gameToken = False):
    """ Run a game of Connect Four, with the two specified players """    
    board = ConnectFourBoard(k = k1)
        
    game = ConnectFourRunner(player1, player2, board=board,game=gameToken)
